...
void Application::insertTestKVPairs() {
    ...
    for (map<string, string>::iterator it = testKVPairs.begin(); it != testKVPairs.end(); ++it) {
        ...
        log->LOG(&mp2[number]->getMemberNode()->addr, "CREATE OPERATION KEY: %s VALUE: %s at time: %d",
...
void Application::deleteTest() {
    ...
    for (int i = 0; i < testKVPairs.size() / 2; i++) {
        ...
        log->LOG(&mp2[number]->getMemberNode()->addr, "DELETE OPERATION KEY: %s VALUE: %s at time: %d",
                 it->first.c_str(), it->second.c_str(), par->getcurrtime());
    ...
    log->LOG(&mp2[number]->getMemberNode()->addr, "DELETE OPERATION KEY: %s at time: %d", invalidKey.c_str(),
             par->getcurrtime());
...
void Application::readTest() {
    ...
    if (par->getcurrtime() == TEST_TIME) {
        ...
        log->LOG(&mp2[number]->getMemberNode()->addr, "READ OPERATION KEY: %s VALUE: %s at time: %d", it->first.c_str(),
                 it->second.c_str(), par->getcurrtime());
    ...
    if (par->getcurrtime() == (TEST_TIME + FIRST_FAIL_TIME)) {
        ...
        if (replicas.size() < (RF - 1)) {
            cout << endl << "Could not find at least quorum replicas for this key. Exiting!!! size of replicas vector: "
                 << replicas.size() << endl;
    ...
    if (par->getcurrtime() >= (TEST_TIME + FIRST_FAIL_TIME + STABILIZE_TIME)) {
        ...
        if (par->getcurrtime() == (TEST_TIME + FIRST_FAIL_TIME + STABILIZE_TIME)) {
            ...
            if (replicas.size() > 2) {
                ...
                cout << endl << "Not enough replicas to fail two nodes. Number of replicas of this key: "
            ...
            if (count == 2) {
                for (int i = 0; i < nodesToFail.size(); i++) {
                    // Fail a node
                    log->LOG(&mp2[nodesToFail.at(i)]->getMemberNode()->addr, "Node failed at time=%d",
                             par->getcurrtime());
            ...
            cout << endl << "Reading a valid key.... ... .. . ." << endl;
            log->LOG(&mp2[number]->getMemberNode()->addr, "READ OPERATION KEY: %s VALUE: %s at time: %d",
                     it->first.c_str(), it->second.c_str(), par->getcurrtime());
    ...
    if (par->getcurrtime() == (TEST_TIME + FIRST_FAIL_TIME + STABILIZE_TIME + STABILIZE_TIME + LAST_FAIL_TIME)) {
        ...
        log->LOG(&mp2[number]->getMemberNode()->addr, "READ OPERATION KEY: %s at time: %d", invalidKey.c_str(),
                 par->getcurrtime());
        // This read should fail since at least quorum nodes are not alive
...
void Application::updateTest() {
    ...
    if (par->getcurrtime() == TEST_TIME) {
        ...
        log->LOG(&mp2[number]->getMemberNode()->addr, "UPDATE OPERATION KEY: %s VALUE: %s at time: %d",
